...
STATES = ['info']
OBJECT_NAME = 'In-Memory DB Versions (v2)'
RETURNED_KEY = 'inmemorydb_versions'
...
DOCUMENTATION = """
module: inmemorydb_version_v2_info
short_description: List supported In-Memory DB versions (DBaaS In-Memory DB v2 API)
description:
     - This is a simple module that supports listing the In-Memory DB versions supported for
       clusters using the DBaaS In-Memory DB v2 API. The region is selected through the
       I(location) option; set I(api_url) (e.g. C(https://in-memory-db.de-fra.ionos.com/v2))
...
def get_objects(module, client):
    return get_paginated(ionoscloud_dbaas_inmemorydb.VersionsApi(client).versions_get, depth=None)
