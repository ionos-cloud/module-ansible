...
OPTIONS = {
    ...
    'db_username': {
        'description': [
            'The username for the initial mongo user. Some system usernames are restricted (e.g. "mongo", '
            '"admin", "standby")'
        ],
        'available': ['present'],
        'required': ['present'],
...
EXAMPLE_PER_STATE = {
  ...
  ''',
    'update': '''- name: Update Mongo Cluster
    mongo_cluster:
...
def create_mongo_cluster(module, dbaas_client, cloudapi_client):
    ...
    mongo_cluster_properties = ionoscloud_dbaas_mongo.CreateClusterProperties(
        mongodb_version=module.params.get('mongodb_version'),
        instances=module.params.get('instances'),
        connections=connections,
        location=module.params.get('location'),
        display_name=display_name,
        maintenance_window=maintenance_window,
        credentials=ionoscloud_dbaas_mongo.DBUser(
...
def update_mongo_cluster(module, dbaas_client):
    maintenance_window = module.params.get('maintenance_window')
    if maintenance_window:
        maintenance_window = dict(module.params.get('maintenance_window'))
        maintenance_window['dayOfTheWeek'] = maintenance_window.pop('day_of_the_week')
    ...
    existing_mongo_cluster_id_by_name = get_resource_id(
        module, mongo_clusters, display_name, [['properties', 'display_name']],
    )

    if (
        mongo_cluster_id is not None and 
        existing_mongo_cluster_id_by_name is not None and
        existing_mongo_cluster_id_by_name != mongo_cluster_id
    ):
...
def main():
    ...
    try:
        if state == 'present':
            module.exit_json(**create_mongo_cluster(module, dbaas_client=dbaas_mongo_api_client,
                                                       cloudapi_client=cloudapi_api_client))
        elif state == 'absent':
            module.exit_json(**delete_mongo_cluster(module, dbaas_mongo_api_client))
        elif state == 'update':
            module.exit_json(**update_mongo_cluster(module, dbaas_mongo_api_client))
    except Exception as e:
        module.fail_json(
