...
def create_postgres_cluster(module, dbaas_client, cloudapi_client):
    ...
    existing_postgres_cluster = get_resource_id(module, postgres_clusters.items, display_name,
                                                [['id'], ['properties', 'display_name']])

    if existing_postgres_cluster is not None \
            and existing_postgres_cluster.metadata.state != 'AVAILABLE' and module.params.get('wait'):
    ...
    datacenter_id = get_resource_id(module, ionoscloud.DataCentersApi(cloudapi_client).datacenters_get(depth=1),
                                    connection['datacenter'])

    if datacenter_id is None:
        module.fail_json('Datacenter {} not found.'.format(connection['datacenter']))

    lan_id = get_resource_id(module, ionoscloud.LANsApi(cloudapi_client).datacenters_lans_get(datacenter_id, depth=1),
                             connection['lan'])
...
def delete_postgres_cluster(module, dbaas_client):
    postgres_cluster_server = ionoscloud_dbaas_postgres.ClustersApi(dbaas_client)
    postgres_cluster_id = get_resource_id(module, postgres_cluster_server.clusters_get(),
...
def restore_postgres_cluster(module, dbaas_client):
    ...
    postgres_cluster_id = _get_dbaas_cluser(postgres_cluster_server.clusters_get(),
                                            module.params.get('postgres_cluster'))
