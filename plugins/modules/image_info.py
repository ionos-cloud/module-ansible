...
def get_objects(module, client):
    images_api = ionoscloud.ImagesApi(api_client=client)

    images = images_api.images_get(depth=module.params.get('depth'))

    try:
        results = list(map(lambda x: x.to_dict(), apply_filters(module, images.items)))
        return {
            'changed': False,
            'results': results
        }
