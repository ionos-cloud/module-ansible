...
DNSAAS_USER_AGENT = 'ansible-module/%s_ionos-cloud-sdk-python-dnsaas/%s' % (
__version__, ionoscloud_dnsaas.__version__)
...
def main():
    ...
    if not HAS_SDK:
        module.fail_json(
            msg='ionoscloud_dnsaas is required for this module, run `pip install ionoscloud_dnsaas`')

    dnsaas_api_client = ionoscloud_dnsaas.ApiClient(get_sdk_config(module, ionoscloud_dnsaas))
    dnsaas_api_client.user_agent = DNSAAS_USER_AGENT

    check_required_arguments(module, OBJECT_NAME)
    try:
        zones = ionoscloud_dnsaas.ZonesApi(dnsaas_api_client).zones_get()
        results = list(map(lambda x: x.to_dict(), apply_filters(module, zones.items)))
