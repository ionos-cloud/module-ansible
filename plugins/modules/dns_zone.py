...
def _create_object(module, client, existing_object=None):
    ...
    try:
        registry = zones_api.zones_put(str(uuid.uuid5(uuid.uuid5(uuid.NAMESPACE_URL, REPO_URL), str(uuid.uuid4()))), zone)
...
def _update_object(module, client, existing_object):
    ...
    try:
        registry = ionoscloud_dns.ZonesApi(client).zones_put(zone_id=existing_object.id, zone_ensure=zone)
