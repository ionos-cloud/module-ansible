...
def get_objects(module, client):
    pccs_api = ionoscloud.PrivateCrossConnectsApi(api_client=client)

    pccs = pccs_api.pccs_get(depth=module.params.get('depth'))

    try:
        results = list(map(lambda x: x.to_dict(), apply_filters(module, pccs.items)))
        return {
            'changed': False,
