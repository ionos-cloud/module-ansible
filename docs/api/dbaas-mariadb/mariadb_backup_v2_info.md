# mariadb_backup_v2_info

This is a simple module that supports listing existing MariaDB Cluster backups using the DBaaS MariaDB v2 API. There is no per-cluster backups endpoint, so when I(mariadb_cluster) is provided the account-wide backup list is filtered by cluster id server-side via the API's filter parameter.
...
<table data-full-width="true">
  ...
  <td>The ID or name of an existing MariaDB Cluster. If set, only backups belonging to this cluster are returned.</td>
  </tr>
  <tr>
